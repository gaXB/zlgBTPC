...
//ZCAN_ReceiveFD_Data	
	ZCAN_Receive_Data pCanObjFD0[2500];
	...
	switch ( nRet )
	{
		...
		//OK BUTTON
			if(m_ComDlg.m_Enable)
			{
				if (m_ComDlg.COMID == 0)
				{
					m_dev = ZCAN_OpenDevice(ZCAN_USBCANFD_200U,0,0);// ZCAN_USBCANFD_200U
				}
				else
				{
					m_dev = ZCAN_OpenDevice(ZCAN_USBCAN2,0,0);
				}
#if 1
				if (m_dev == 0x0000)
				{
					MessageBox(L"deveice id is NULL!");
					return;
				}
				...
				if(_pPro == NULL)
				{
					MessageBox(L"Property's NULL!");
					return;
				}
				if (m_ComDlg.COMID == 0)
				{
					_pPro->SetValue("0/canfd_abit_baud_rate","500000");
					_pPro->SetValue("0/canfd_dbit_baud_rate","500000");
				}
				else
				{
					ZCAN_SetValue(m_dev, "0/baud_rate", "500000");
					ZCAN_SetValue(m_dev, "1/baud_rate", "500000");
				}
#endif
#if 0
if (0 == ZCAN_SetValue(m_dev, "0/canfd_abit_baud_rate", "500000")) {

 }
 // 设置通道 0 数据域波特率为 4M
 if (0 == ZCAN_SetValue(m_dev, "0/canfd_dbit_baud_rate", "1000000")) {
				...
				{
					MessageBox(L"Start-CAN0 failed!");
					ReleaseIProperty(_pPro);
					return;
				}
...
UINT CCailbration_CANDlg::ReceiveThread(void *param)
{
	...
	//	Deal_UDSNetLay();
		...
		{}
			...
			if (dev_ch1 != 0)
			{
				len=ZCAN_GetReceiveNum(dev_ch1,0);

				if (len > 0)
				{
					len = ZCAN_Receive(dev_ch1, pCanObjFD0, len, 50);
				}
				else{}
			}
			else
		...
		//	box->InsertString(box->GetCount(),str);
			for(i=0;i<len;i++)
			{
				...
				str+=tmpstr;
 				box->InsertString(box->GetCount(),str);*/
#if 1
				ReceivedID = GET_ID(pCanObjFD0[i].frame.can_id);
				//if(frameinfo[i].RemoteFlag==0 && (frameinfo[i].ID == ID_DIAG_PHYSIC|| frameinfo[i].ID == ID_DIAG_FUNCT|| frameinfo[i].ID == ID_DIAG_CLM))
				if (IS_RTR(pCanObjFD0[i].frame.can_id)== 0 && (ReceivedID == ID_DIAG_PHYSIC || ReceivedID == ID_DIAG_FUNCT|| ReceivedID == ID_DIAG_CLM))
				{
					if (dlg->Eeprom_Window_Flg == WINDOUS_FLG_BOOT)
					{
						str="RX:";
						tmpstr.Format(L"ID %02x  Data:", ReceivedID );
						str += tmpstr;
						//if(frameinfo[i].DataLen>8)
						//	frameinfo[i].DataLen=8;
						for(int j=0;j<8;j++)
						{
							tmpstr.Format(L"%02x ",pCanObjFD0[i].frame.data[j]);
							str+=tmpstr;
						}
				...
				{
					if (ReceivedID == ID_DIAG_CLM)
					{
						NetLay_ReciveDiag((unsigned char)(ID_DIAG_CLM&0x00ff), pCanObjFD0[i].frame.data);    
					}
					else if (ReceivedID == ID_DIAG_PHYSIC)
					{
						NetLay_ReciveDiag((unsigned char)(ID_DIAG_PHYSIC&0x00ff), pCanObjFD0[i].frame.data); 
					}
				}
// 				if (box->GetCount()>50)
// 				{
...
void ShowData(ZCAN_Transmit_Data sVData)
{
	CString str;
   ...
   //CTestDlg *dlg=(CTestDlg*)param;	
	...
	str.Format(L"TX : ID - %X, DATA - %x, %x, %x, %x, %x, %x, %x, %x", sVData.frame.can_id ,sVData.frame.data[0], sVData.frame.data[1], sVData.frame.data[2],
		sVData.frame.data[3], sVData.frame.data[4], sVData.frame.data[5], sVData.frame.data[6], sVData.frame.data[7]);
...
#if 1
	ZCAN_Transmit_Data canfd_data;
	
	if (DiagID)
	{
		canfd_data.frame.can_id =MAKE_CAN_ID(ID_DIAG_FUNCT, ID_FRAMESIZE, 0, 0) ; // 填写第一帧的ID 
	}
	else
	{
		canfd_data.frame.can_id =MAKE_CAN_ID(ID_DIAG_PHYSIC, ID_FRAMESIZE, 0, 0) ; // 填写第一帧的ID 
	}

	for(int i=0;i<8;i++)
		canfd_data.frame.data[i]=NetData[i];
//	if (TIMEID) return ;
	canfd_data.transmit_type = 0; //正常发送

	if( pWnd->m_ComDlg.m_Enable ==0)
		return;
	canfd_data.frame.can_dlc = 8;
	//canfd_data.frame.
	if (ZCAN_Transmit( dev_ch1, &canfd_data, 1)==1)
	{
	//	m_sendcnt += 1;
		...
		if (pWnd->Eeprom_Window_Flg == WINDOUS_FLG_BOOT)
		{
			CString str;
			CString str1;
			str.Format(L"TX : ID - %X, DATA - ", canfd_data.frame.can_id );
			
			for (int i = 0; i < 8; i++)
			{
				str1.Format(L"%x,", canfd_data.frame.data[i]);
				str += str1;
			}
	...
	if (DiagID)
	{
		canfd_data.frame.can_id = ID_DIAG_FUNCT; // 填写第一帧的ID 
	}
	else
	{
		canfd_data.frame.can_id = ID_DIAG_PHYSIC; // 填写第一帧的ID 
	}
...
//	if (TIMEID) return ;
	...
	if (ZCAN_TransmitFD( dev_ch1, &canfd_data, 1)==1)
	{
	//	m_sendcnt += 1;
